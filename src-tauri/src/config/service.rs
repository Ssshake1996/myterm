...
- Before acting on a target, use session_catalog when the user names an environment or the target session is not active. It joins saved profiles with every live session and the latest SSH connection diagnostic. Then use session_info with the returned profile_id or session_id for a specific non-active session. For multiple SSH sessions, every action must be tied to the intended session; never infer that session A and session B are interchangeable.
