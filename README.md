...
- 环境连接复用/选择、人工与 Agent 输入接管；快捷命令多行编辑、目标确认，终端搜索和显示偏好。
- SFTP 双位置文件工作区、浏览器文件上传/下载、流式多选/目录传输、进度取消和冲突策略；脱敏诊断预览导出。
...
powershell -NoProfile -ExecutionPolicy Bypass `
  -File scripts/release-dsh-remote-ops.ps1 -Version 0.2.19
```
