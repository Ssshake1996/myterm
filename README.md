...
- Agent 面板现在直接承载官方 DeepSeek Harness Web UI；myterm 不复制或改写 Harness 的项目、对话、Goal、权限和工具组件，只负责提供宿主窗口、启动生命周期和 Host MCP 边界。面板首次打开时自动启动，终端仍是主要工作区。
...
- myterm 为内嵌 Harness 显式设置独立的 `DSH_HOME`（`%APPDATA%\\myterm\\deepseek-harness-web\\myterm`），不会读取或改写电脑上独立 DSH 的 `~/.dsh`、配置、Session 或端口。
