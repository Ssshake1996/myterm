...
- `02-architecture.md` 全文(重点:内核类型、前端 IPC 契约的会话部分、进程模型与数据面、错误处理约定)
- `src-tauri/src/types.rs`、`src-tauri/src/main.rs`

## 任务

实现 SSH 会话的完整生命周期:用 russh 建连接、认证、开 shell channel(请求 pty,TERM=xterm-256color),把输出以二进制推给前端 Channel 并同时写入环形缓冲;实现会话相关的全部 Tauri 命令与事件。`SessionTarget::Local` 分支本任务返回 "not implemented" 错误(M5 实现)。
...
- `snapshot_lines` 去除 CSI/OSC 转义序列后按行返回最近 n 行(简单状态机即可,不要求完整终端仿真;覆盖 `ESC [ ... 字母` 与 `ESC ] ... BEL/ST` 两类)。此缓冲是 M6 AI 抓屏的数据源。
...
- 按 `profile.target` 分派:`Ssh` 走 ssh.rs;`Local` 暂返回错误(M5 接入 local.rs,`SessionManager` 接口设计时就要容纳两者)。
...
- 用例至少覆盖:
  ...
  8. `Local` target → 明确的 "not implemented" 错误。
- 无 Docker 时整组测试显式 skip(打印原因),不得假通过。

## 禁止事项

- 不要实现 SFTP(M4)、本地终端(M5)、真实 vault(M3)。
