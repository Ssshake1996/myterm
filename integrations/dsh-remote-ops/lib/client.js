window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const empty = { groups: [], environments: [], quickGroups: [], quickCommands: [], sessions: [], events: [], bound: false, pluginName: "dsh-remote-ops", pluginVersion: "0.2.9", update: { currentVersion: "0.2.9", latestVersion: "0.2.9", updateAvailable: false } };
    ...
    const terminalVisibleText = (value, rows = 40, columns = 160) => {
      ...
      const lines = [...scrollback, ...screen.map(lineText)];
    ...
    function RemoteOpsPanel({ sessionId }) {
      ...
      const activeSession = snapshot.sessions.find((item) => item.sessionId === activeSessionId) ?? snapshot.sessions[0]; const activeEnvironment = snapshot.environments.find((item) => item.id === (activeSession?.environmentId ?? selectedEnv)); const localActive = activeSession?.kind === "local"; const terminalReady = localActive || snapshot.bound;
      ...
      const terminalOutput = useMemo(() => terminalVisibleText(activeTerminalFrame?.raw ?? ""), [activeTerminalFrame?.raw]);
      useEffect(() => {
      ...
      const terminalInputCapture = h("textarea", { ref: terminalInputRef, className: "dsh-remote-ops__inputCapture", value: terminalInput, readOnly: !snapshot.bound && !snapshot.sessions.some((item) => item.kind === "local" && item.status?.kind === "running"), spellCheck: false, autoCapitalize: "off", autoCorrect: "off", "aria-label": "终端输入", onChange: (event) => { const value = event.target.value; if (value) queueRawInput(value); setTerminalInput(""); }, onKeyDown: (event) => { const sequence = keySequence(event); if (sequence !== undefined) { event.preventDefault(); queueRawInput(sequence); setTerminalInput(""); } }, onPaste: (event) => { event.preventDefault(); queueRawInput(event.clipboardData.getData("text")); } });
      const terminal = h("section", { className: "dsh-remote-ops__terminal" },
        ...
        activeSession ? h("div", { className: "dsh-remote-ops__screen", onClick: () => terminalInputRef.current?.focus() },
          h("pre", { className: "dsh-remote-ops__output", ref: outputRef, onWheel: markUserScrollIntent, onPointerDown: markUserScrollIntent, onScroll: (event) => { const node = event.currentTarget; const nearBottom = node.scrollHeight - node.scrollTop - node.clientHeight < 48; if (userScrollIntent.current) stickToBottom.current = nearBottom; else if (nearBottom) stickToBottom.current = true; } }, terminalOutput || "等待终端输出…"),
          terminalInputCapture,
          terminalOutput ? null : h("div", { className: "dsh-remote-ops__screenHint" }, "点击输入 · Tab 补齐 · Ctrl+C 中断"),
        ) : h("div", { className: "dsh-remote-ops__screen dsh-remote-ops__screenEmpty", onClick: () => terminalInputRef.current?.focus() }, h("div", { className: "dsh-remote-ops__empty" }, h("div", null, h("div", { style: { fontSize: "22px", marginBottom: "8px" } }, "›_"), h("div", null, snapshot.bound ? "输入 ssh [user@]host 并按 Enter 连接，或从环境抽屉选择环境。" : "请先在当前 DSH 对话发送消息，等待 Agent 初始化。"), pendingSshInput ? h("pre", { className: "dsh-remote-ops__pendingCommand" }, pendingSshInput) : null)), terminalInputCapture, h("div", { className: "dsh-remote-ops__screenHint" }, snapshot.bound ? "输入 SSH 命令 · Enter 连接" : "等待 DSH Agent 初始化")),
      );
      ...
      return h("div", { className: "dsh-remote-ops" }, h("div", { className: "dsh-remote-ops__head" }, h("strong", null, "Remote Ops"), h("span", { className: "dsh-remote-ops__version" }, `v${snapshot.pluginVersion ?? update.currentVersion ?? "?"}`), h("button", { className: "dsh-remote-ops__headEnv", onClick: () => setDrawer(true), title: activeEnvironment?.name ?? "选择环境" }, activeEnvironment?.name ?? "选择环境"), h("span", { className: "dsh-remote-ops__muted" }, snapshot.sessions.length ? `${snapshot.sessions.length} 个终端` : (snapshot.bound ? "0 个终端" : "等待本地 CMD")) , update.updateAvailable ? h("button", { className: "dsh-remote-ops__tiny dsh-remote-ops__update", disabled: updateBusy, "aria-busy": updateBusy, onClick: () => void installUpdate() }, updateBusy ? "升级中…" : `升级 v${update.latestVersion}`) : h("button", { className: "dsh-remote-ops__tiny", disabled: updateBusy, "aria-busy": updateBusy, onClick: () => void checkUpdate(false) }, updateBusy ? "检查中…" : "检查更新"), h("button", { className: "dsh-remote-ops__tiny", disabled: busy || refreshBusy, "aria-busy": refreshBusy, onClick: () => void refreshNow() }, refreshBusy ? "刷新中…" : "刷新"), h("span", { className: "dsh-remote-ops__headNotice", "data-error": actionNotice.includes("失败") }, actionNotice)), error ? h("div", { className: "dsh-remote-ops__error" }, h("div", { className: "dsh-remote-ops__errorBody" }, error), h("button", { className: "dsh-remote-ops__errorClose", title: "关闭错误", "aria-label": "关闭错误", onClick: () => setError("") }, "×")) : null, h("div", { className: "dsh-remote-ops__workspace" }, h("nav", { className: "dsh-remote-ops__rail", "aria-label": "Remote Ops 导航" }, [["environments", "环境"], ["quick", "快捷命令"], ["sftp", "SFTP"], ["diagnostics", "诊断"]].map(([id, label]) => h("button", { key: id, "data-active": id === "environments" ? drawer : id === "quick" ? quickOpen : module === id, title: label, "aria-label": label, onClick: () => id === "environments" ? (setDrawer((value) => !value), setModule("terminal")) : id === "quick" ? (setDrawer(false), setModule("terminal"), setQuickOpen((value) => !value)) : (setDrawer(false), setModule((value) => value === id ? "terminal" : id)) }, glyph[id])), h("div", { className: "dsh-remote-ops__railSpacer" }), h("button", { className: "dsh-remote-ops__tiny", title: "刷新", onClick: () => void refreshNow() }, "↻")), h("main", { className: "dsh-remote-ops__main" }, terminal, quickPanel), envDrawer, aux));
    }
