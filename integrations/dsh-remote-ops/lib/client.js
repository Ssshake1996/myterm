window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const empty = { groups: [], environments: [], quickGroups: [], quickCommands: [], sessions: [], events: [], bound: false, pluginName: "dsh-remote-ops", pluginVersion: "0.2.3", update: { currentVersion: "0.2.3", latestVersion: "0.2.3", updateAvailable: false } };
    ...
    function RemoteOpsPanel({ sessionId }) {
      const [snapshot, setSnapshot] = useState(empty); const [drawer, setDrawer] = useState(false); const [module, setModule] = useState("terminal"); const [quickOpen, setQuickOpen] = useState(true); const [environmentForm, setEnvironmentForm] = useState(null); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [update, setUpdate] = useState(empty.update); const [updateBusy, setUpdateBusy] = useState(false); const [selectedEnv, setSelectedEnv] = useState(""); const [activeSessionId, setActiveSessionId] = useState(""); const [input, setInput] = useState(""); const [quickGroup, setQuickGroup] = useState(""); const [search, setSearch] = useState(""); const [sftpPath, setSftpPath] = useState("."); const [entries, setEntries] = useState([]); const outputRef = useRef(null); const stickToBottom = useRef(true); const quickRef = useRef(null);
      const refresh = useCallback(async () => { try { const value = await request(`/api/dsh-remote-ops/state${sessionId ? `?sessionId=${encodeURIComponent(sessionId)}` : ""}`); setSnapshot(value); if (value.update) setUpdate(value.update); setError(""); } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); } }, [sessionId]);
      useEffect(() => { void refresh(); const timer = setInterval(() => void refresh(), 1500); return () => clearInterval(timer); }, [refresh]);
      const checkUpdate = useCallback(async () => { setUpdateBusy(true); try { const value = await request("/api/dsh-remote-ops/update"); setUpdate(value); setError(""); return value; } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); return undefined; } finally { setUpdateBusy(false); } }, []);
      useEffect(() => { void checkUpdate(); }, [checkUpdate]);
      useEffect(() => { if (!selectedEnv && snapshot.environments[0]) setSelectedEnv(snapshot.environments[0].id); if (!quickGroup && snapshot.quickGroups[0]) setQuickGroup(snapshot.quickGroups[0]); if (activeSessionId && !snapshot.sessions.some((item) => item.sessionId === activeSessionId)) setActiveSessionId(""); }, [activeSessionId, quickGroup, selectedEnv, snapshot]);
      useEffect(() => { const node = outputRef.current; if (node && stickToBottom.current) node.scrollTop = node.scrollHeight; }, [activeSessionId, snapshot.sessions]);
      ...
      const installUpdate = async () => { if (!update.updateAvailable || updateBusy) return; if (!window.confirm(`安装 Remote Ops v${update.latestVersion}？安装完成后需要重启 DSH。`)) return; setUpdateBusy(true); try { const value = await request("/api/dsh-remote-ops/update", { method: "POST" }); setUpdate(value); setError(""); } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); } finally { setUpdateBusy(false); } };
      const activeSession = snapshot.sessions.find((item) => item.sessionId === activeSessionId) ?? snapshot.sessions[0]; const activeEnvironment = snapshot.environments.find((item) => item.id === (activeSession?.environmentId ?? selectedEnv));
      ...
      const send = async (text) => { if (!snapshot.bound) { setError("当前对话尚未初始化 Harness Agent，请先发送一条 DSH 消息。"); return; } let target = activeSession?.sessionId; if (!target && selectedEnv) { const opened = await action({ action: "open", environment: selectedEnv }); target = opened?.sessionId; if (target) setActiveSessionId(target); } if (!target) { setError("请先选择并连接 SSH 环境。"); return; } await action({ action: "send", session: target, text, submit: true }); };
      ...
      const environmentFormView = environmentForm ? h("form", { className: "dsh-remote-ops__form", onSubmit: saveEnvironmentForm },
        ...
        h("label", null, "Harness 凭据引用", h("input", { value: environmentForm.passwordRef, onChange: (event) => updateEnvironmentForm("passwordRef", event.target.value), placeholder: environmentForm.credentialConfigured ? "已配置，留空保持不变" : "不填写明文密码" })),
        ...
        h("div", { className: "dsh-remote-ops__formNote" }, environmentForm.credentialConfigured ? "凭据已配置；密码留空保持原密码，输入新密码会更新凭据。" : "SSH 密码会保存到 Harness credentials；环境 JSON 只保存凭据引用，不保存明文。"),
      ...
      const envDrawer = drawer ? h("aside", { className: "dsh-remote-ops__drawer" },
        h("div", { className: "dsh-remote-ops__drawerHead" },
          ...
          h("button", { className: "dsh-remote-ops__tiny", onClick: () => setDrawer(false) }, "×"),
        ),
      ...
      const quickPanel = quickOpen ? h("section", { className: "dsh-remote-ops__quick", ref: quickRef },
      ...
      const terminal = h("section", { className: "dsh-remote-ops__terminal" },
        h("div", { className: "dsh-remote-ops__terminalHead" },
          h("strong", null, activeEnvironment ? `${activeEnvironment.name} · ${activeEnvironment.host}` : "SSH 终端"),
          h("small", null, activeSession ? activeSession.status?.kind ?? "运行中" : "未连接"),
          h("button", { className: "dsh-remote-ops__tiny", disabled: busy || !activeSession, onClick: () => void action({ action: "signal", session: activeSession?.sessionId, signal: "SIGINT" }) }, "中断"),
          h("button", { className: "dsh-remote-ops__tiny", onClick: () => setDrawer(true) }, "环境"),
        ),
        ...
        activeSession ? h("pre", { className: "dsh-remote-ops__output", ref: outputRef, onScroll: (event) => { const node = event.currentTarget; stickToBottom.current = node.scrollHeight - node.scrollTop - node.clientHeight < 48; } }, activeSession.viewport || "等待终端输出…") : h("div", { className: "dsh-remote-ops__empty" }, h("div", null, h("div", { style: { fontSize: "22px", marginBottom: "8px" } }, "›_"), h("div", null, snapshot.bound ? "请从环境抽屉选择 SSH 环境并连接。" : "请先在当前 DSH 对话发送消息，等待 Agent 初始化。"))),
        h("div", { className: "dsh-remote-ops__input" },
          h("textarea", { value: input, onChange: (event) => setInput(event.target.value), onKeyDown: (event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); const text = input; setInput(""); if (text) void send(text); } }, placeholder: activeSession ? "输入命令，Enter 发送；Shift+Enter 换行" : "连接环境后输入命令" }),
          h("button", { disabled: busy || !input || !snapshot.bound, onClick: () => { const text = input; setInput(""); void send(text); } }, "发送"),
        ),
      );
      ...
      return h("div", { className: "dsh-remote-ops" }, h("div", { className: "dsh-remote-ops__head" }, h("strong", null, "Remote Ops"), h("span", { className: "dsh-remote-ops__version" }, `v${snapshot.pluginVersion ?? update.currentVersion ?? "?"}`), h("button", { className: "dsh-remote-ops__headEnv", onClick: () => setDrawer(true), title: activeEnvironment?.name ?? "选择环境" }, activeEnvironment?.name ?? "选择环境"), h("span", { className: "dsh-remote-ops__muted" }, snapshot.bound ? `${snapshot.sessions.length} 个终端` : "等待 Agent"), update.updateAvailable ? h("button", { className: "dsh-remote-ops__tiny dsh-remote-ops__update", disabled: updateBusy, onClick: () => void installUpdate }, updateBusy ? "升级中…" : `升级 v${update.latestVersion}`) : h("button", { className: "dsh-remote-ops__tiny", disabled: updateBusy, onClick: () => void checkUpdate }, updateBusy ? "检查中…" : "检查更新"), h("button", { className: "dsh-remote-ops__tiny", disabled: busy, onClick: () => void refresh }, "刷新")), error ? h("div", { className: "dsh-remote-ops__error" }, error) : null, h("div", { className: "dsh-remote-ops__workspace" }, h("nav", { className: "dsh-remote-ops__rail", "aria-label": "Remote Ops 导航" }, [["environments", "环境"], ["quick", "快捷命令"], ["sftp", "SFTP"], ["diagnostics", "诊断"]].map(([id, label]) => h("button", { key: id, "data-active": id === "environments" ? drawer : id === "quick" ? quickOpen : module === id, title: label, "aria-label": label, onClick: () => id === "environments" ? (setDrawer((value) => !value), setModule("terminal")) : id === "quick" ? (setDrawer(false), setModule("terminal"), setQuickOpen((value) => !value)) : (setDrawer(false), setModule((value) => value === id ? "terminal" : id)) }, glyph[id])), h("div", { className: "dsh-remote-ops__railSpacer" }), h("button", { className: "dsh-remote-ops__tiny", title: "刷新", onClick: () => void refresh }, "↻")), h("main", { className: "dsh-remote-ops__main" }, terminal, quickPanel), envDrawer, aux));
    }
    function RemoteOpsTitle() { return h("span", null, "Remote Ops"); }
    function RemoteOpsLaunch({ wide, onClick, label }) { return h("button", { type: "button", className: "dsh-remote-ops__launchControl", title: label, "aria-label": label, onClick }, h("span", { className: "dsh-remote-ops__launchControlIcon", "aria-hidden": true }, "SSH"), wide ? h("span", null, label) : null); }
    ...
    function apply(ctx) {
      ...
      const showLauncherError = (message) => { console.warn(`[dsh-remote-ops] ${message}`); if (typeof document === "undefined") return; let node = document.getElementById("dsh-remote-ops-launch-error"); if (!node) { node = document.createElement("div"); node.id = "dsh-remote-ops-launch-error"; node.style.cssText = "position:fixed;right:18px;bottom:18px;z-index:99999;max-width:360px;padding:10px 12px;border:1px solid rgba(217,45,32,.35);border-radius:8px;background:var(--dsw-alias-bg-base,#fff);color:var(--dsw-alias-state-error-primary,#d92d20);box-shadow:0 8px 28px rgba(16,24,40,.18);font:13px/1.4 system-ui,sans-serif"; document.body.appendChild(node); } node.textContent = message; node.hidden = false; window.clearTimeout(Number(node.dataset.timer)); node.dataset.timer = String(window.setTimeout(() => { node.hidden = true; }, 5000)); };
      let launcherTimer;
      const openRemoteOps = () => {
