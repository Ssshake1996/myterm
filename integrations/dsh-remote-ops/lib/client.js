window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function RemoteOpsPanel({ sessionId }) {
      ...
      const terminalInputCapture = h("textarea", { ref: terminalInputRef, className: "dsh-remote-ops__inputCapture", value: terminalInput, readOnly: !terminalReady, spellCheck: false, autoCapitalize: "off", autoCorrect: "off", "aria-label": "终端输入", onChange: (event) => { const value = event.target.value; if (value) queueRawInput(value); setTerminalInput(""); }, onKeyDown: (event) => { const sequence = keySequence(event); if (sequence !== undefined) { event.preventDefault(); queueRawInput(sequence); setTerminalInput(""); } }, onPaste: (event) => { event.preventDefault(); queueRawInput(event.clipboardData.getData("text")); } });
