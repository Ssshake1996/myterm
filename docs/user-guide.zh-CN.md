...
本说明书适用于 myterm 0.12.1。myterm 将服务器管理、SSH 与本地终端、SFTP、快捷命令和 DeepSeek Harness Linux 运维 Agent 放在同一个紧凑工作区中。
...
- Agent 面板直接加载官方 DeepSeek Harness Web UI，首次打开应用时自动启动；终端是主要工作区。用户仍可手动收起，展开后可在 Harness 自己的项目、对话、Goal 和权限界面中工作。
- 每个 Harness 对话都可绑定一个或多个已保存 SSH 环境，也可随时解绑。绑定由 myterm Host MCP 后端强制执行，未绑定的远程环境不会被工具调用。
- 内嵌 Harness 使用 myterm 独立的 `DSH_HOME`：`%APPDATA%\\myterm\\deepseek-harness-web\\myterm`。它与电脑上单独运行的 DSH 不共用配置、Session、端口或进程环境。
