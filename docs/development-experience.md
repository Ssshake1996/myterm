...
## 23. 原生 DeepSeek Provider 与能力界面收敛（0.11.2）

### 问题确认

v0.11.1 已把 Agent Loop 迁到官方 Harness，但模型层仍通过通用兼容 Provider 转换配置，安装资源和 AI 界面因此继续暴露与实际内核不一致的认证、协议和路由概念。这个问题会让用户误以为 Agent 仍有第二套模型内核，也让 Base URL、认证头和推理参数存在多条运行路径。

### 方案比较

| 方案 | 优点 | 缺点 | 结论 |
|---|---|---|---|
| 保留通用兼容 Provider | 旧网关配置可继续使用 | 继续维护多协议分支；界面无法准确表达 Harness 能力 | 不采用 |
| 在宿主重新实现 DeepSeek 请求 | 协议完全可控 | 再次形成独立模型传输和流式循环，背离上游同步目标 | 不采用 |
| 官方 `dsh-llm-deepseek` + 固定路由 | 配置、流式、重试与推理强度由上游负责；运行路径唯一 | 旧 AI 配置需要重新录入；网关必须提供 Provider 支持的 Bearer 接口 | 采用 |

### 实现决定

1. Harness profile 改用官方 `dsh-llm-deepseek`，ACP 固定选择 `deepseek-official`；Rust 只注入 `apiKeyEnv`、Base URL、推理强度和当前模型。
2. 删除宿主旧流式聊天 IPC、认证模式、分析模型角色、人工上下文参数和兼容 Provider 字段。AI schema 升到 v6，开发阶段直接清空旧 AI 服务，不保留迁移分支。
3. AI 设置只呈现 DeepSeek 官方/网关、主模型/备用模型、推理强度、获取模型和测试模型。Agent 面板直接展示 Session、Goal、Checkpoint、Compaction、Skill 与 Host MCP 状态。
4. 运行时审计只检查可长期复用的架构契约，例如官方包版本一致、必要插件/工具包存在、ACP 路由和配置注入正确。针对某次历史残留的依赖名或文件名不建立永久专用门禁，避免修复越多、规则越臃肿。

### 验证与可复用经验

- 原生 Provider check 和 ACP smoke 验证 profile 可加载、固定路由可创建会话；Rust 测试验证 Provider JSON、推理强度、系统 Prompt 和主/备用模型归一化。
- 前端测试覆盖原生 Provider 配置、能力状态条、模型删除后的主模型提升和 JSON 预览；构建仍由统一发布脚本执行一次完整测试、资源准备、安装包验证与内存采样。
- 可复用经验：门禁应保护稳定边界，而不是纪念某次故障。一次性残留应在实现和依赖树中删除；长期门禁只表达产品必须持续成立的契约。

## 24. 纯 DeepSeek Harness 插件边界与常驻终端工作区（0.2.0）

### 问题确认

仓库同时包含旧 myterm 桌面端、旧运行时、实验代码和 DSH 插件，导致产品边界、构建入口和发布对象不一致。插件 UI 也仍然使用模块切换页签，环境、终端和快捷命令无法同时形成稳定的工作区。

### 实现决定

1. 主分支删除旧桌面应用、Tauri 宿主、旧 Harness runtime、实验代码和旧 myterm 文档；Git 历史保留，后续仓库只维护 `@dsh/remote-ops`。
2. Remote Ops 使用 DSH 官方右侧 Sidebar 扩展点，不注入全局主面板，因此主对话不会被替换或隐藏。
3. Sidebar 内采用窄导航栏 + 可隐藏环境抽屉。抽屉关闭时终端获得完整空间，抽屉打开时以覆盖层管理环境，终端组件不卸载。
4. 终端是主工作区，使用标签承载多个 SSH 会话；快捷命令固定在终端下方，命令分组和命令正文不再占用终端上方空间。
5. 环境和快捷命令分别按分组目录保存，凭据继续使用 Harness credentials 引用，不把密码写入 JSON。
6. 环境和命令的增删改由 Sidebar 直接调用插件状态接口；SSH、终端输入和 SFTP 仍要求活动 Harness Agent，会话未初始化时只展示等待状态。

### 验证与可复用经验

- 插件静态检查必须覆盖 Host、Client 和烟测；打包前再验证 `npm pack --dry-run` 的文件边界。
- UI 验收要同时确认主对话仍可见、右侧 Sidebar 正常打开、环境抽屉可隐藏、终端区域在无连接时也存在。
- “始终显示终端”不等于“所有辅助功能永久展开”。窄导航栏和抽屉把低频管理动作收起，终端输出和输入保持稳定，符合 Xshell 会话树、标签和命令集的工作习惯。
- 产品边界切换时，删除当前分支中的旧代码即可实现可维护的纯插件仓库；保留 Git 历史比重写历史更可恢复，也不影响后续 Release 资产。

## 25. 跨 DSH 版本激活与插件自更新（0.2.1）

### 问题确认

部分电脑安装插件后，Web 启动停留在 `pending (waiting for services:
sidebarRight, sidebarRightTabs)`。根因是插件把可选的浏览器 Sidebar 服务声明成了顶层硬依赖；当 DSH 版本较旧、配置不是 Web，或服务尚未被加载时，Cordis 会让插件保持等待状态并阻塞启动。

### 实现决定

1. Client 顶层只依赖稳定的 `slots`、`locale` 服务；右侧 Sidebar 通过 `ctx.inject(["sidebarRight", "sidebarRightTabs"], ...)` 动态挂载。
2. 缺少右侧 Sidebar 时，插件本身仍然激活，启动按钮不会造成 Web boot 失败；服务出现后由 Cordis 自动重新挂载面板。
3. 插件后端暴露更新检查和安装接口。更新源固定为 GitHub 官方仓库最新 Release，安装命令由插件在活动 DSH profile 中调用 `pnpm add`，不接受任意外部 URL。
4. Sidebar 显示当前版本、检查更新和一键升级状态；安装后明确提示重启 DSH，避免在线替换正在执行的模块。

### 验证与可复用经验

- 静态烟测检查版本常量、更新入口和动态 Sidebar 依赖；正式发布前仍执行一次完整检查与打包。
- 兼容性问题优先降低不必要的顶层依赖，再通过运行时服务注入恢复能力；这样不会因低频 UI 服务缺失拖垮整个 Host/Web 启动。
- 自更新必须限制仓库和资产来源，并把“安装成功”和“当前进程已切换”分开表达；后者只能通过重启可靠完成。

## 26. Sidebar 直接编辑与主题一致性（0.2.3）

1. 环境编辑属于高频操作，改为 Sidebar 内一次提交的表单；浏览器 `prompt` 只适合低频原型，不适合作为正式配置入口。
2. 快捷命令导航必须有可见状态变化，改为控制终端下方命令栏的显示/收起，并提供栏内收起按钮。
3. 启动按钮对服务加载竞态做短时重试；仍不可用时显示具体原因，避免跨电脑出现“点击无反应”。
4. 终端不再固定深色背景，统一使用 DSH 语义背景、边框和文字 token；命令输入显式左对齐，减少主题和布局漂移。
...
## 29. Sidebar 快捷命令区与凭据抽象（0.2.6）

1. 快捷命令属于终端的辅助工作区，固定在终端下方但不能抢占终端的主要空间；使用独立的拖拽分隔条调整高度，设置最小高度与窗口一半的最大高度，双击可恢复默认值。
2. 环境表单不暴露 Harness 内部凭据引用。用户只处理 SSH 密码或私钥，后端负责生成、保留和更新凭据引用，避免把内部实现细节变成使用门槛。
3. 检查更新和刷新必须有可观察反馈：按钮进入进行中状态，成功显示结果，失败同时保留具体错误信息；自动轮询不得覆盖用户主动操作的反馈。
4. 启动按钮遵循 DSH Web 的主题变量，但通过图标、层次化边框、悬停和按下状态表达“打开远程运维 Sidebar”，不改变启动逻辑。

## 31. 无 SSH 时的本地 CMD 工作区与断线回退（0.2.8）

### 问题确认

Remote Ops 之前在没有活动 SSH 会话时只显示等待提示，用户无法直接在插件内执行 `ssh`、网络检查或本地准备命令；SSH 断开后也没有稳定的本地终端承接后续操作。

### 实现决定

1. 插件初始化时通过 DSH 的 `subprocess.spawnTerminal` 启动一个插件级本地终端。Windows 使用隐藏的 `cmd.exe /D /Q /K`，启动时切换到 UTF-8；非 Windows 使用当前 Shell 的交互模式。
2. 本地终端工作目录固定为插件用户数据根目录 `$DSH_HOME/remote-ops`，环境 JSON、快捷命令和命令行操作使用同一根目录，避免落入 DSH 安装目录或系统目录。
3. 本地终端不依赖活动 Harness Agent，因此插件刚打开时即可输入命令；本地输入通过原始 PTY 通道发送，支持回车、Tab、方向键、Ctrl+C 和交互式 `ssh`。
4. 保存环境打开的 SSH 仍由 owner-scoped Harness terminal 管理。SSH 会话退出后，快照隐藏已退出会话，界面自动选择本地 CMD；通过本地 CMD 启动的 OpenSSH 子进程则由 cmd 自然回到提示符。
5. 本地 CMD 退出后自动重新启动，避免插件再次打开时停留在无终端状态；关闭插件时设置 disposal 标志，禁止退出回调重新拉起进程。

### 验证与可复用经验

- 使用真实 DSH Web `3080` 端口验证了插件未绑定 Agent 时仍显示“本地 CMD”、工作目录为 `C:\Users\ssshake\.dsh\remote-ops`，并实际执行 `echo` 命令。
- 执行 `exit` 后验证本地 CMD 自动恢复，状态重新变为 running；PTY 输出中的 Windows 光标定位序列在 UI 层转换为可读换行，避免命令结果与提示符粘连。
- “无 SSH 时提供本地终端”应复用宿主的 subprocess/PTY 能力，不额外创建隐藏窗口或第二套 shell 进程管理；本地终端是 UI 工作区能力，不把本机命令误暴露为远程 SSH Agent 工具。

## 32. 终端屏幕模型、完整滚动与增量输出（0.2.9）

### 问题确认

真实 DSH Web 3080 验证确认了两个独立问题：输出元素使用 `content-box` 且高度为 100%，上下内边距使其实际高度比父容器多 20px，导致最后一行和滚动条底部被裁剪；Windows ConPTY 初始化会发送 40 行屏幕快照及光标回定位序列，线性去除 ANSI 后会把空屏行当成历史输出，形成大段留白。输入路径还会在每个短批次后刷新完整状态，终端历史越长，重复传输和渲染成本越高。

### 实现决定

1. 输出区域改为绝对填充、`border-box` 和稳定滚动条槽，元素高度与父容器严格一致；有终端内容时不再用提示层遮挡最后一行。
2. 显示层使用无额外依赖的轻量 VT 屏幕模型，处理清屏、光标定位、回车覆盖、退格、制表、擦除和滚屏；ConPTY 的空屏快照不会进入可见历史。
3. Host 使用带绝对字符游标的有界输出缓冲。前端只拉取当前游标之后的增量，游标过期时才重置为受控的最近历史。
4. 输出等待改为长轮询：无新内容时只保留一个等待请求，有输出立即唤醒；状态元数据独立低频刷新，不再携带终端正文。
5. 键盘输入按短窗口合并并严格串行发送，不在每个输入批次后刷新完整状态，避免并发请求乱序和输入卡顿。
6. 自动跟随只在用户位于底部时生效；用户滚轮上移或拖动滚动条后保留阅读位置，重新滚到底部后自动恢复跟随。
7. 输出缓冲按增量字节计数，超过上限时一次性收缩到保留水位，删除原先反复拼接和计算整个 4MB 缓冲区的二次复杂度路径。

### 验证与可复用经验

- 真实 DSH Web 3080 中连续输出 2000 行，约 0.67 秒进入页面；最终显示 2003 行，末尾提示符完整可见，`clientHeight` 与父容器均为 616px，底部间隙为 0。
- 滚轮上移 12000px 后，新输出不会抢回底部；滚动到底部再输出时，自动跟随恢复且底部间隙保持 0。
- 14 秒空闲采样中，DSH Host 私有内存变化 0MB、句柄变化 0、CPU 增量 0 秒；终端只保留一个长轮询请求，不再持续高频空轮询。
- 终端输出不能作为普通日志字符串展示。即使暂不引入完整 xterm 依赖，也必须正确区分屏幕状态、滚动历史和增量传输，否则 CSS 修复无法解决 ConPTY 空白、重复内容和光标定位问题。

## 33. 回归测试先行与发布门禁（0.2.10）

### 问题确认

仅依赖人工验证会把缺陷发现推迟到用户环境；而只保留静态烟测又无法覆盖环境持久化、客户端行为和插件契约。新增功能或重构必须有可重复的回归入口，发布流程不能绕过已有能力检查。

### 实现决定

1. 插件测试拆成后端单元、状态持久化集成、客户端行为和插件契约四层，统一由 `npm test` 串行执行。
2. `npm run check` 先做 `node --check`，再调用完整 `npm test`；Release 脚本只调用这一门禁，不维护第二套重复测试命令。
3. 后端测试覆盖环境字段与凭据引用校验、分组/快捷命令落盘重载、UTF-8 输出缓冲、有界增量游标、长轮询超时和 Abort 取消。
4. 客户端行为测试直接从生产 `client.js` 提取并执行 VT 屏幕模型与 SSH 命令解析，锁定 ConPTY 空屏、光标覆盖、滚屏和参数透传行为。
5. 契约测试保护所有远程终端、Multi-SSH、快捷命令、SFTP、诊断、状态/动作/更新路由，以及滚动条、增量轮询和串行输入等长期边界。
6. 真实 DSH Web/3080 页面验收保留为宿主集成检查：连续输出、滚动位置、末行显示、面板切换和空闲资源采样必须记录在 Release 说明中。

### 验证与可复用经验

- 四层自动化回归通过后才允许提交和发布；首次运行发现的是测试断言跨 VM 原型和终端光标前导空格的测试问题，修正后产品实现无需回退。
...
## 36. 终端光标定位与 SSH locale 初始化（0.2.13）

### 问题确认

终端光标固定在左下角不是有效的终端反馈：命令输入、提示符、光标移动和多行输出都会让这个位置失真。SSH 建立时把 `export LANG=...` 写入 PTY 也会被远端 Shell 回显，用户会误以为插件替他输入了一条命令。

### 实现决定

1. VT 屏幕模型在返回可见文本的同时返回当前光标行列；客户端把光标作为对应文本行中的内联元素渲染，不再使用左下角绝对定位。
2. SSH `shell` 请求通过 `ssh2` 的 channel environment 选项发送 `LANG`、`LC_ALL` 和 `LC_CTYPE`，不再向远端 stdin 写入 locale 初始化命令。
3. 回归测试覆盖普通文本、清屏定位后的光标位置、shell locale 配置，以及源码中禁止重新出现可见 `export LANG` 写入。

### 验证与可复用经验

- `npm run check` 通过，未引入 xterm 或其他运行时重量级依赖。
- 本轮 3080 页面验收未完成：检查时本机没有 3080 监听进程；不能把宿主集成行为写成已验证。
- SSH locale 初始化应优先使用协议级环境变量请求；只有在宿主或远端明确不支持时，才考虑严格限定的回显过滤。

## 37. 环境连接生命周期与双栏 SFTP（0.2.14）

### 问题确认

环境 ID 不能同时承担“业务归属”和“PTY 唯一名称”两个职责。旧实现把环境 ID直接作为 PTY name，导致宿主保留旧连接时新连接触发 `DUPLICATE_NAME`；同时插件只按自身内存判断 running，重启或宿主主动结束 PTY 后会继续显示灰色 running。旧 SFTP 只提供窄侧栏目录读取，无法完成常用的本地/远端文件传输。

### 实现决定

1. 保存环境只作为会话归属信息；每次打开连接生成唯一 PTY name，并把环境对象随 spawn 请求传给 SSH backend。旧版以环境 ID命名的 PTY 仍可通过前缀/精确匹配恢复。
2. 同一 owner+环境最多 3 个活动连接。并发打开共享一个 opening promise，顺序打开按需创建新连接；环境 ID 在存在多个连接时不再隐式选择，必须使用明确 sessionId。
3. `terminals.list(owner)` 作为连接事实来源。宿主列表中消失的插件会话从快照移除，环境状态由实际连接数计算；UI 在环境下列出每个连接并支持独立释放。
4. 批量命令先为每个环境打开一个连接，再复用该 sessionId，避免每条命令重复占用连接额度。
5. SFTP 支持本地目录读取、单文件上传和下载，限制单文件大小；客户端使用完整主区双栏布局，移动端退化为上下分栏。

### 验证与可复用经验

- 回归测试覆盖独立 PTY 名称、3 个连接上限、指定连接释放、宿主 PTY 消失后的状态回收、本地目录读取和 SFTP UI/路由契约。
- `npm run check` 通过；本轮 3080 页面验收未完成，因为检查时本机没有 3080 监听进程。
- 不能把“环境 running”当作连接永久存在的标记；它必须是 owner 视角下实时连接计数的派生状态。文件传输应先提供清晰的本地/远端边界，再考虑目录递归、拖放和断点续传等更高成本能力。

## 34. 跨电脑终端输入激活与高可见操作控件（0.2.11）

### 问题确认

部分电脑打开插件后终端停留在等待输入状态，但键盘输入没有反应。问题集中在客户端输入激活边界：隐藏 textarea 只通过外层点击间接聚焦，且本地终端启动阶段与 Harness Agent 绑定状态共用提示，导致用户无法判断到底是在等待本地 PTY 还是等待 Agent。顶部中断、环境、检查更新和刷新动作也使用了过小的紧凑按钮；环境抽屉打开后没有形成明确的二次收回路径，关闭按钮不够醒目。

### 实现决定

1. 增加 `terminalInputEnabled` 行为判定：Harness 未绑定时，只要本地 session 状态为 `running`，终端输入仍然可写；本地 session 尚未运行时才保持只读。
2. 本地终端就绪或切换活动终端后，通过 `requestAnimationFrame` 主动聚焦输入捕获层；保留 `pointer-events:none`，避免输入层拦截终端滚轮和文本阅读。
3. 环境按钮使用函数式状态切换，打开和收回共享同一个动作；顶部操作按钮使用统一的较大点击区域，环境抽屉关闭按钮使用 DSH 错误色和明确的 `aria-label`。
4. 回归测试验证用户可观察的输入就绪判定和 UI 行为契约，不把真实 SSH、SFTP 或模型连接成功伪装成自动化结果。

### 验证与可复用经验

- `npm run check` 通过，客户端行为和契约测试新增回归用例均通过。
- 3080 端口确认由本机 DSH Web 监听；由于验收浏览器无登录态，HTTP 返回 `401 Unauthorized`，因此不能把本次结果记为完成页面验收。
- 终端输入这类问题应分开验证“PTY 是否已运行”“Harness 是否已绑定”“输入捕获层是否获得焦点”三个状态，不要用一个等待文案覆盖不同故障阶段。

## 38. 生成 PTY 名称反查与 SFTP 类型图标（0.2.15）

### 问题确认

环境定义和 PTY 会话是两层对象。宿主根据插件生成的唯一 PTY 名称调用 SSH backend 时，若 backend 只按环境 ID 或名称查找，就会把已存在的环境误报为 `REMOTE_ENV_NOT_FOUND`。SFTP 列表同时把“文件/目录”作为每一行的可见前缀，增加了扫描噪声，也不符合文件管理器的常见交互习惯。

### 实现决定

1. `spawnBackend` 在显式环境对象、直接环境映射和普通环境查找之后，使用 `findEnvironmentBySessionName` 解析生成的 PTY 名称；多个环境 ID 存在前缀重叠时优先匹配最长 ID，避免连接到错误环境。
2. 回归测试直接调用 SSH backend，验证生成 PTY 名称最终使用保存环境的 host、port 和 username，不把测试限制在辅助解析函数。
3. SFTP 条目保留文件名和大小，使用颜色化图标区分目录与文件，不再在文件名之前渲染“文件/目录”文字；图标隐藏于辅助技术的名称之外，文件名仍是主要可访问名称。

### 验证与可复用经验

- `npm run check` 必须继续作为唯一发布门禁；环境解析回归应覆盖 backend 调用路径，而不是只断言字符串前缀。
- 本轮 3080 页面验收中，现有保存环境 `笔记本` 成功建立 SSH PTY 并显示 Ubuntu 远端提示符；SFTP 成功列出本地和远端目录，文件名显示为独立名称并带类型图标。
- `REMOTE_ENV_NOT_FOUND` 与真实 SSH 连接失败必须分阶段区分：前者表示插件没有解析到环境，后者才表示网络、凭据或远端阶段失败。

## 39. Agent 工具无损 JSON 与环境名称简化（0.2.16）

### 问题确认

DSH Web 另一台主机使用最新插件调用 `remote_environment_list` 时，工具已经被找到并执行，但 DSH 在校验返回值时报告 `value is not lossless JSON`。原因是快照对象在没有密码或本地错误时包含 `undefined` 属性；普通 `JSON.stringify` 会忽略这些属性，但工具输出校验不会。环境编辑表单还暴露了用户不需要理解的内部环境 ID。

### 实现决定

1. 所有插件 Agent 工具统一在注册层把返回值归一化为 JSON 可表示的数据：对象省略 `undefined` 属性，数组使用 `null`，非有限数字使用 `null`，BigInt 转为字符串，循环结构直接报出明确错误。
2. 环境编辑界面不再展示内部 ID。新环境由插件生成内部 ID，编辑已有环境时通过内部状态保留原 ID；用户只处理名称、主机、账号和连接配置。
3. 环境名称可以留空，保存时使用主机地址；重复名称拒绝保存并返回 `REMOTE_ENV_NAME_EXISTS`，避免 UI 中出现无法区分的环境。
4. 不做旧版本数据迁移或兼容层；内部 ID 只作为运行时和持久化关联字段存在。

### 验证与可复用经验

- 回归测试直接检查环境快照经过归一化后不含 `undefined`，并覆盖无 ID、空名称、自动命名和重复名称。
- 契约测试保护环境表单不再渲染“环境 ID”，并确保工具注册层统一调用 JSON 归一化。
- `value is not lossless JSON` 表示工具已执行但输出契约不满足，不应误判为插件未加载；排查时应优先检查工具返回对象中的 `undefined`、非有限数字和宿主不接受的特殊值。

## 40. 终端与 Agent 同流、增量续读和阅读位置（0.2.17）

### 问题确认

本地 CMD 原先写入后立即返回旧 viewport，Agent 的 read/signal 又只走 SSH 查找，因而用户看到的新输出不等于工具当时拿到的内容。缓冲区还会在有效游标与末尾相距超过单页大小时直接跳到尾部；重建输出流若长度不变，前端无法识别旧缓存。输入队列只有文本，切换终端时可能使用错误的目标闭包。

### 实现决定

1. 复用已有 SendOperation，不引入第二套模型循环；本地 CMD 与 SSH 共用等待/取消契约，工具与 HTTP 路由读取相同 TerminalOutputBuffer。发送默认返回 16 Ki 码元的新输出，read 用绝对游标加 streamId 续读，保留范围内顺序分页并明确 hasMore。
2. 每次流重建生成新 streamId，过期或替换明确 reset/truncated；前端拒绝不连续拼接。readOutput 遵守宿主“消费增量”契约，不能每次重放整个命令输出。
3. 不通过向任意 CLI 注入 shell 标记猜测命令完成，不引入沉重的服务端终端渲染器。该方案兼容交互式程序且维护成本低，但只能返回 completion=unknown 和实际原始流；需要调用者观察结果，不能宣称提供可靠退出码或屏幕语义。
4. 输入排队时保存 session 与 owner，状态和帧按 owner 隔离；明确插件 local-cmd 不等于 DSH 内置 bash/pwsh。宿主工具取消信号必须传入等待实现。
5. UI 用宿主图标和 36px 控件，区分输出连接、Agent 绑定、等待状态。快捷区默认收起；上移阅读不抢滚动，新输出提供回到底部入口。
6. SFTP 会替换终端 DOM，但不关闭终端进程。不能在 effect cleanup 读取已脱离文档的 scrollTop（浏览器会返回 0）；必须在真实 scroll 事件时保存位置。ResizeObserver 仅在用户选择跟随底部时恢复末行。

### 验证与边界

- 先补失败回归再修改实现；`npm run check` 通过：22 项单元、9 项客户端行为、7 项契约和烟测。注册工具与路由使用 fake 宿主验证数据契约，不冒充真实网络或模型。
- 3080 实际模型首次误用参数 sessionId，工具阶段原始错误为 `Error: REMOTE_ENV_NOT_FOUND: Environment not found: undefined`；改用文档规定的 session 后发送/续读成功。新增缺少目标时的 `REMOTE_SESSION_REQUIRED` 回归，不添加错误参数别名。
- 真实模型发送 echo 标记后，工具返回 88 个码元的新输出；游标从 458 到 546，续读从 546 返回空文本且 nextOffset 不变，界面显示同一命令和标记。数字是该次样本，不是性能基准。
- 3080 验证 240 行输出、上移阅读时追加输出、SFTP 返回保持 scrollTop、390px 窄屏和桌面末行、快捷命令实际回显、Ctrl+C、local-cmd 退出后重建。真实 SSH 回显和 SFTP 两侧目录已确认，测试连接及临时快捷数据已清理。
- 未验证跨机差异、真实操作系统输入法候选提交、SFTP 上传下载，也未测得可报告的模型耗时或 token 降幅。共享数据源不等于模型必然读取、理解或正确使用参数。
- 发布后以正式 Release 包重新安装本机 web profile 并重启 3080，页面显示 v0.2.17；中文、连续空格回显以及末行复核通过，启动 stderr 为空。GitHub 包 SHA-256 与本地发布包一致；文档交接另行提交，不再创建新 Release。

## 41. 输入协作与流式文件工作区（0.2.19）

- 基线为 `3ca7761` / v0.2.18。已有共享插件导航是有效改动，不能因 README 尚为 v0.2.17 而撤回；以 package/version/git 为事实来源。
- 人工编辑与 Agent 写入不能只靠前端按钮互斥。后端记录输入持有方，人工写入后阻止工具继续写；显式接管结束等待，不伪装成 Ctrl+C。宿主可能包装 SendOperation，只暴露 done；应使用插件持有的 backend active operation，未知 backend 则明确拒绝不支持的操作。
- 恢复已有 Harness 会话使用公开 `sessionController.resolveAgent`，不自行创建 Agent/权限循环。没有已保存 sessionId 的新草稿不能被伪造为 owner。
- SFTP 使用 Node 流和背压，暂存文件完成后发布；取消只清理本次暂存，已完成文件不回滚。目录递归、冲突和所有权检查用真实临时文件测试，不为单一历史问题建立永久额外门禁。
- 浏览器设备与 DSH 主机必须明确区分。真实宿主下载最初返回 `HTTP 400`、空响应体，阶段为 HTTP bridge，未进入插件下载处理。读取宿主实现发现 streaming 模式会给 GET 附加 body；分开 POST streaming 上传与 GET buffered 请求注册即可，GET 响应仍然流式。不改宿主，不引入第二 HTTP 服务。
- 文件请求仅传端点身份和路径，不把目录列表/勾选状态随每次请求发送。输出搜索预计算匹配集合，渲染逐行查集合，避免匹配项较多时平方级扫描。
- 自动化检查通过：38 单元/传输/路由、15 客户端、7 契约和烟测；真实 3080 验证模型被人工输入权拒绝，交还后发送游标 324→405，续读 405→405。SFTP 3,145,751 字节上传、同远端跨目录复制、下载逐字节一致。连接复用、3 个上限和指定释放通过。
- 桌面/390px 窄屏、中文文本/连续空格、快捷命令多行与预览取消、搜索/字号已验证。未验证 OS 输入法候选提交、另一客户端差异和两台不同 SSH 服务互传；不能把模型总结、单机跨目录或模拟测试当作这些场景的成功证据。
- 正式发布提交 `8cb4bfb`，Tag `dsh-remote-ops-v0.2.19`；安装正式包并重启 3080 后，CMD 回显、复制输出、SFTP 面板往返、390px 无溢出和真实 SSH 复用/上限/指定释放复核通过。包 SHA256 为 `9cb41c5356f1adc39ad7b9152d45a831d4d125bdfd47195be6ac87179bb1ecdc`，与 GitHub asset digest 一致。浏览器控制台的预期 HTTP 400 对应第四连接被 `REMOTE_SESSION_LIMIT` 拒绝，不能混同未捕获页面异常；后者为零。启动仅有宿主 SQLite ExperimentalWarning。

## 42. 可辨识连接、文件恢复与独立命令（0.2.20）

- 基线为 `150181c` / v0.2.19，工作区干净。用户确认的八项范围一起交付：连接备注/编号、断线恢复、工具回执、文件传输反馈/重试、路径工作区、窗口输入权、终端兼容性评估和独立命令。不做旧数据迁移。
- 编号不能用当前数组下标计算，释放中间连接会使其他标签漂移；使用 owner/环境上的递增编号。断线保留当前目标和末次屏幕，重连要显式触发且不重放输入。输入队列/粘贴锁定 owner、目标、clientId 和 streamId；窗口输入权由后端校验，不只禁用按钮。
- 回执仅记录实际工具返回范围和时间。读 HTTP 输出不是模型读取；共享输出流也不保证模型读取、理解或完成命令。回执按 owner/stream 隔离，新流不得沿用旧回执。
- 传输先形成有界文件清单，保留每项状态，重试时只重置未完成项。完成/跳过项不重传，不承诺跨进程续传或回滚。覆盖预览读取两端元数据，运行时重新检查源和目标；UI 完成刷新仅匹配当前端点/目录，避免把用户拉回旧目录。
- 真实验收暴露两个路径交互问题：切换端点后的异步加载可能覆盖快速输入的新路径；定位文件可能重置排序。先保留失败回归，再分别禁用加载中的路径输入、复用目标面板偏好。滚动在 scroll 事件保存，不能在卸载后读取 DOM，路径偏好不保存旧目录行或勾选。
- 独立命令单独使用 Harness subprocess 和已有 SSH client.exec，不给 PTY 注入完成标记，不绕过宿主另起模型/权限循环。独立进程不会继承交互 shell 的 cd/export；stdout/stderr 有界且 UTF-8 截断不输出半个字符，超时/取消与真实退出分开。SSH 关闭通道不保证远端进程死亡，结果如实标记。
- 先评估宿主能力再决定终端方案：当前没有运行时 PTY resize API，保留 160×40，未引入 xterm。真实 top 不一定进入备用屏，只隐藏光标；需保留网格和顶端，不能靠视觉换行重排。ESC(B 是字符集选择而不是可打印 B；vim 退出必须恢复普通屏幕。
- 真实下载曾出现 `MaxListenersExceededWarning: Possible EventEmitter memory leak detected. 11 drain listeners added to [Gzip]`，阶段为 HTTP 响应背压。启用 trace 后保留原始堆栈：`compression/index.js:148:23` 转发 drain，`dsh-client-connection/lib/index.js:99:7` 注册 once。插件二进制下载加 `Cache-Control: no-store, no-transform`，避免不必要的 HTTP 压缩；不修改宿主、不调大监听上限。新增下载表示契约回归后，两次 3,145,751 字节下载均保持原始 Content-Length、无 Content-Encoding、逐字节一致，日志未再出现该警告。
- `npm run check`：50 项后端/传输/路由/独立命令、22 项客户端、7 项契约及烟测通过。开发过程中失败回归分别验证了排序、路径输入、VT 和下载表示问题，未删除失败用例或用实现细节替代行为断言。
- 3080 真实模型调用独立命令返回 exitCode=7、stdout=MODEL020-OK、stderr=MODEL020-ERR、durationMs=127；随后读取 PTY 只有提示符，已展开检查原始工具结果。真实 SSH exec 返回非零退出码 9 与中文；3 条连接、稳定标签、逐个释放、双窗口接管、退出保留/重连、top/vim 通过。
