...
| 永久保留所有旧 schema 迁移 | 任意旧版本可直接升级 | 开发阶段长期积累死代码和测试矩阵，增加内核分支 | v0.9.14 只保留一版迁移，v0.9.15 删除 |
...
- **v0.9.15 必做清理**：删除 `AppConfig.profiles` 兼容字段、`migrate_legacy_profiles`、迁移报告 IPC/前端提示及其专用测试；保留环境文件加载、严格校验和原子写入。
