...
| 模式 | 默认行为 | 适用场景 |
|---|---|---|
| 只读 | 只允许采集信息，禁止修改 | 排查、审计、先看方案 |
| 用户确认 | 低风险读取自动执行，变更和敏感读取确认 | 默认模式 |
| 完全授权 | 除硬拒绝规则外自动执行，不再逐次确认 | 明确承担风险的受控批处理 |

权限判定采用 `deny > ask > allow`，同时检查工具、解析后的命令、规范化路径、目标服务器、登录用户和服务器环境标签。完全授权仍受硬拒绝规则保护；用户确认模式提供“仅本次允许”和“拒绝”。
...
- `full_access`、Skill、Hook 和 MCP 都不能覆盖上述硬拒绝规则；生产/root 场景应由用户明确选择完全授权并保留审计。
