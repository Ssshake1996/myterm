...
```powershell
cd F:\myterm
npm run release -- -Version 0.11.0 -RunRustTests
```
...
5. 执行 `dsh-codex-agent` 固定门禁：Core Rust 测试、当前源码的 Release N-API 重建、TypeScript build/typecheck、Biome 和 N-API 集成测试。测试不会加载旧 `native-dist` 二进制。
6. 执行桌面宿主 `cargo fmt --check` 和 `cargo check -j 1`；传入 `-RunRustTests` 时再单线程执行宿主 Rust 全量测试。Core Rust 测试已包含在上一步，不重复运行。
7. 使用 `npm run build:release` 生成新的 NSIS 安装器和便携 ZIP。Rust 构建固定使用 `CARGO_BUILD_JOBS=1`，不复用旧版本二进制。
...
```powershell
npm run release -- -Version 0.11.0 -SkipPublish -RunRustTests
```
